      ...
      mDisplay(display)
{
    ...
    if (result != 0) {
        qFatal("QBB: failed to query display rotation, errno=%d", errno);
    }
    ...
    if (result != 0) {
        qFatal("QBB: failed to query display size, errno=%d", errno);
    }
    ...
    if (result != 0) {
        qFatal("QBB: failed to query display physical size, errno=%d", errno);
    }
    ...
    if (getuid() == 0) {
        ...
        if (result != 0) {
            qFatal("QBB: failed to set window z-order, errno=%d", errno);
        }
    }
    ...
    if (result != 0) {
        qFatal("QBB: failed to set window colour, errno=%d", errno);
    }
    ...
    if (result != 0) {
        qFatal("QBB: failed to set window source position, errno=%d", errno);
    }
    ...
    // this MUST be done before the first post!
    errno = 0;
    result = screen_create_window_group(mAppWindow, windowGroupName());
    if (result != 0) {
        qFatal("QBB: failed to create app window group, errno=%d", errno);
    }

    // window also won't be visible unless it's been posted so do this now
    errno = 0;
    int dirtyRect[] = {0, 0, val[0], val[1]};
...
void QBBScreen::createDisplays(screen_context_t context)
{
    ...
    if (result != 0) {
        qFatal("QBB: failed to query display count, errno=%d", errno);
    }
    ...
    if (result != 0) {
        qFatal("QBB: failed to query displays, errno=%d", errno);
    }
...
{
    // available geometry = total geometry - keyboard (but only on the main display).
    int keyboardHeight = 0;
    if (this == sScreens[0] && BlackBerry::Keyboard::instance().isVisible())
        keyboardHeight = BlackBerry::Keyboard::instance().keyboardHeight();
...
void QBBScreen::setRotation(int rotation)
{
    ...
    if (mCurrentRotation != rotation) {
        ...
        if (result != 0) {
            qFatal("QBB: failed to set window rotation, errno=%d", errno);
        }
        ...
        if (orthogonal(mCurrentRotation, rotation)) {
            ...
            if (result != 0) {
                qFatal("QBB: failed to flush context, errno=%d", errno);
            }
        }
...
void QBBScreen::addWindow(QBBWindow* window)
{
    ...
    if (sChildren.contains(window))
        qFatal("QBBScreen: Attempt to add a window that is already in the hierarchy!");
...
void QBBScreen::removeWindow(QBBWindow* window)
{
    ...
    if (!sChildren.removeOne(window))
        qFatal("QBBScreen: Attempt to remove window that is not in the hierarchy!");
}
...
{
#if defined(QBBSCREEN_DEBUG)
    qDebug() << "QBBScreen::updateHierarchy this="<<this;
#endif
